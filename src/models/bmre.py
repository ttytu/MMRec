...
class BMRE(GeneralRecommender):
    def __init__(self, config, dataset):
        super(BMRE, self).__init__(config, dataset)
        ...
        # ================== REVIEW EMBEDDING ==================
        # ======================================================
        if self.u_r_feat is not None:
            self.u_r_embedding = nn.Embedding.from_pretrained(self.u_r_feat, freeze=False)
            self.u_r_trs = nn.Linear(self.u_r_feat.shape[1], self.feat_embed_dim)
            nn.init.xavier_normal_(self.u_r_trs.weight)
        if self.i_r_feat is not None:
            self.i_r_embedding = nn.Embedding.from_pretrained(self.i_r_feat, freeze=False)
            self.i_r_trs = nn.Linear(self.i_r_feat.shape[1], self.feat_embed_dim)
            nn.init.xavier_normal_(self.i_r_trs.weight)
        # ======================================================
        # ================== REVIEW EMBEDDING ==================
    ...
    def calculate_loss(self, interactions):
        ...
        # ================== REVIEW EMBEDDING ==================
        # ======================================================
        u_r_online, i_r_online = None, None
        if self.u_r_feat is not None:
            u_r_online = self.u_r_trs(self.u_r_embedding.weight)
        if self.i_r_feat is not None:
            i_r_online = self.i_r_trs(self.i_r_embedding.weight)
        # ======================================================
        # ================== REVIEW EMBEDDING ==================
        ...
        with torch.no_grad():
            ...
            # ======================================================
            if self.u_r_feat is not None:
                u_r_target = u_r_online.clone()
                u_r_target = F.dropout(u_r_target, self.dropout)
            if self.i_r_feat is not None:
                i_r_target = i_r_online.clone()
                i_r_target = F.dropout(i_r_target, self.dropout)
            # ======================================================
            # ================== REVIEW EMBEDDING ==================
        ...
        # ================== REVIEW EMBEDDING ==================
        # ======================================================
        loss_u_r, loss_i_r = 0.0, 0.0
        if self.u_r_feat is not None:
            u_r_online = self.predictor(u_r_online)
            u_r_online = u_r_online[users, :]
            u_r_target = u_r_target[users, :]
            loss_u_r = 1 - cosine_similarity(u_r_online, u_target.detach(), dim=-1).mean()
        if self.i_r_feat is not None:
            i_r_online = self.predictor(i_r_online)
            i_r_online = i_r_online[items, :]
            i_r_target = i_r_target[items, :]
            loss_i_r = 1 - cosine_similarity(i_r_online, i_target.detach(), dim=-1).mean()

            loss_ui_r = 1 - cosine_similarity(u_r_online, i_r_target.detach(), dim=-1).mean()
            loss_iu_r = 1 - cosine_similarity(i_r_online, u_r_target.detach(), dim=-1).mean()
        # ======================================================
        # ================== REVIEW EMBEDDING ==================
